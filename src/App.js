...
const App = () => {
  ...
  return (
    <div>
      ...
      <div className="task-list" style={{ display: 'flex', flexWrap: 'wrap' }}>
        {filteredTodos.map((todo, index) => (
          <Card key={`todo-item-${index}`} todo={todo} onDelete={handleDelete} onEdit={handleEdit} />
        ))}
      </div>
    </div>
  );
